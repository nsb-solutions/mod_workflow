...
$event = \mod_workflow\event\course_module_instance_list_viewed::create(array(
    'context' => $modulecontext
));
$event->add_record_snapshot('course', $course);
$event->trigger();

$PAGE->set_url('/mod/workflow/index.php', array('id' => $id));
...
foreach ($workflows as $workflow) {
    if (!$workflow->visible) {
        $link = html_writer::link(
            new moodle_url('/mod/workflow/view.php', array('id' => $workflow->coursemodule)),
            format_string($workflow->name, true),
            array('class' => 'dimmed'));
    } else {
        $link = html_writer::link(
            new moodle_url('/mod/workflow/view.php', array('id' => $workflow->coursemodule)),
    ...
    if ($course->format == 'weeks' or $course->format == 'topics') {
        $table->data[] = array($workflow->section, $link);
    } else {
