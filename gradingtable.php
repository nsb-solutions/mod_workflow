...
class assign_grading_table extends table_sql implements renderable {
     ...
     *
     * @param workflow $workflow The assignment class
    ...
    public function __construct(workflow $workflow,
        ...
        $this->output = $PAGE->get_renderer('mod_workflow');

        $urlparams = array('action' => 'grading', 'id' => $workflow->get_course_module()->id);
        $url = new moodle_url($CFG->wwwroot . '/mod/workflow/view.php', $urlparams);
        $this->define_baseurl($url);

        // Do some business - then set the sql.
        $currentgroup = groups_get_activity_group($workflow->get_course_module(), true);

        if ($rowoffset) {
            $this->rownum = $rowoffset - 1;
        }

        $users = array_keys( $workflow->list_participants($currentgroup, true));
        if (count($users) == 0) {
        ...
        $params = array();
        $params['workflowid1'] = (int)$this->workflow->get_instance()->id;
        $params['workflowid2'] = (int)$this->workflow->get_instance()->id;
        $params['workflowid3'] = (int)$this->workflow->get_instance()->id;
        $params['newstatus'] = REQUEST_SUBMISSION_STATUS_PENDING;

        $extrauserfields = get_extra_user_fields($this->workflow->get_context());

        //$fields = user_picture::fields('u', $extrauserfields) . ', ';
        $fields = fields('u', $extrauserfields) . ', ';
